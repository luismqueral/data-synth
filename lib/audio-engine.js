...
export class AudioEngine {
    constructor() {
        ...
        // Visualizer state (stereo oscilloscope)
        this.analyser = null;
        ...
        // Idle animation state (for when not playing)
    ...
    setupVisualizer(canvas) {
        // Store canvas reference (needed for both idle and active visualization)
        ...
        // If audio context exists, set up analysers for real-time audio visualization
        if (this.audioContext) {
            // Create stereo oscilloscope (split L/R channels)
            this.splitter = this.audioContext.createChannelSplitter(2);
            this.analyser = this.audioContext.createAnalyser();
            this.analyserRight = this.audioContext.createAnalyser();
            
            // Configure for smooth waveform display
            const fftSize = 1024; // Good balance of detail and performance
            const smoothing = 0.3; // Slight smoothing removes jitter
            ...
            this.analyserRight.smoothingTimeConstant = smoothing;
            this.analyserRight.minDecibels = -100;
            this.analyserRight.maxDecibels = -10;
            
            this.dataArray = new Uint8Array(fftSize);
            this.dataArrayRight = new Uint8Array(fftSize);
            
            console.log('✅ Stereo Oscilloscope initialized, fftSize =', fftSize);
            
            // Connect: reverb → splitter → [L analyser, R analyser] → destination
            if (this.reverbWetGain) {
                ...
                this.reverbWetGain.connect(this.splitter);
                this.splitter.connect(this.analyser, 0); // Left
                this.splitter.connect(this.analyserRight, 1); // Right
                this.analyser.connect(this.audioContext.destination);
                this.analyserRight.connect(this.audioContext.destination);
            } else {
                this.analyser.connect(this.audioContext.destination);
                console.warn('⚠️ Effects chain not initialized');
            }
        } else {
            console.log('✅ Visualizer canvas ready (idle animation only - audio context not yet created)');
        }
        
        // Start idle animation immediately (works without audio context)
        if (!this.animationId) {
     ...
     */
    /**
     * Generate idle waveform animation when not playing
     * 
     * Creates organic, slowly moving waveforms using multiple sine waves
     * at different frequencies to simulate "living" audio visualization
     * 
     * Algorithm: Layered sine waves with different frequencies and phases
     * - Creates Lissajous-like patterns that never repeat exactly
     * - Slow phase shifts create smooth, meditative motion
     * - Multiple harmonics add visual complexity
     */
    drawIdleWaveform(width, height, halfHeight, time) {
        // Time-based animation (runs continuously)
        const t = time * 0.0003; // Slow movement
        
        // Number of samples for smooth curves
        const numSamples = 512;
        const sliceWidth = width / numSamples;
        
        // Waveform style (slightly muted for idle state)
        ...
        // LEFT channel (top half) - combination of sine waves
        ...
        for (let i = 0; i < numSamples; i++) {
            ...
            const wave1 = Math.sin(x * Math.PI * 2 + t);
            const wave2 = Math.sin(x * Math.PI * 3 + t * 1.3) * 0.5;
            const wave3 = Math.sin(x * Math.PI * 5 + t * 0.7) * 0.25;
            const combined = (wave1 + wave2 + wave3) / 1.75;
            
            // Scale to fit in top half with gentle amplitude
            const y = (halfHeight / 2) - (combined * (halfHeight / 2) * 0.6);
            
            i === 0 ? this.visualizerCtx.moveTo(i * sliceWidth, y) : this.visualizerCtx.lineTo(i * sliceWidth, y);
        }
        this.visualizerCtx.stroke();
        
        // RIGHT channel (bottom half) - different phase for stereo effect
        this.visualizerCtx.beginPath();
        for (let i = 0; i < numSamples; i++) {
            const x = i / numSamples;
            
            // Different frequencies for right channel (creates stereo interest)
            const wave1 = Math.sin(x * Math.PI * 2 + t * 1.1 + Math.PI / 4);
            const wave2 = Math.sin(x * Math.PI * 4 + t * 0.9) * 0.5;
            const wave3 = Math.sin(x * Math.PI * 6 + t * 1.2) * 0.25;
            const combined = (wave1 + wave2 + wave3) / 1.75;
            
            // Scale to fit in bottom half
            const y = halfHeight + (halfHeight / 2) - (combined * (halfHeight / 2) * 0.6);
    ...
    drawVisualizer(isPlaying) {
        // Always run visualizer (idle animation when not playing)
        if (!this.visualizerCanvas || !this.visualizerCtx) {
            return null;
        }
        
        // Schedule next frame (continuous animation)
        ...
        // Draw labels
        this.visualizerCtx.fillStyle = '#999';
        this.visualizerCtx.font = '10px IBM Plex Sans, sans-serif';
        this.visualizerCtx.fillText('Left Channel', 8, 14);
        this.visualizerCtx.fillText('Right Channel', 8, halfHeight + 14);
        
        // Center dividing line
        this.visualizerCtx.strokeStyle = '#ddd';
        this.visualizerCtx.lineWidth = 1;
        this.visualizerCtx.beginPath();
        this.visualizerCtx.moveTo(0, halfHeight);
        this.visualizerCtx.lineTo(width, halfHeight);
        this.visualizerCtx.stroke();
        
        // Draw either real audio data or idle animation
        if (isPlaying && this.analyser && this.dataArray) {
            // PLAYING: Show real audio waveform from analyser
            ...
            const amp = 15; // Dramatic amplitude
            
            // LEFT channel (top half)
            this.visualizerCtx.beginPath();
            for (let i = 0; i < this.dataArray.length; i++) {
                const v = (this.dataArray[i] - 128) / 128; // -1 to +1
                const y = (halfHeight / 2) - (v * (halfHeight / 2) * amp);
                i === 0 ? this.visualizerCtx.moveTo(i * sliceWidth, y) : this.visualizerCtx.lineTo(i * sliceWidth, y);
            }
            this.visualizerCtx.stroke();
            ...
            if (this.dataArrayRight) {
                ...
                for (let i = 0; i < this.dataArrayRight.length; i++) {
                    const v = (this.dataArrayRight[i] - 128) / 128;
                    const y = halfHeight + (halfHeight / 2) - (v * (halfHeight / 2) * amp);
                    i === 0 ? this.visualizerCtx.moveTo(i * sliceWidth, y) : this.visualizerCtx.lineTo(i * sliceWidth, y);
                }
                this.visualizerCtx.stroke();
            }
        } else {
            // IDLE: Show animated procedural waveforms
            const currentTime = Date.now() - this.idleAnimationStartTime;
            this.drawIdleWaveform(width, height, halfHeight, currentTime);
        }
