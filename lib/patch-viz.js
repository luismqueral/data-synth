...
export class PatchViz {
    ...
    _drawNodes(container, svgWidth, mappings, isPlaying) {
        ...
        // Value highlight background (only for data fields on left)
        nodeGroups.filter(d => d.type === 'data')
            .append('rect')
            .attr('class', 'value-highlight')
            .attr('id', d => `highlight_${d.id}`)
            .attr('x', -35)
            .attr('y', 8)
            .attr('width', 70)
            .attr('height', 14)
            ...
            if (node.type === 'data') {
                ...
                if (valueEl) {
                    if (value !== undefined && value !== null) {
                        ...
                        // Flash yellow highlight background (data fields only)
                        const highlightEl = document.getElementById(`highlight_${node.id}`);
                        if (highlightEl) {
                            // Add flash class
