 ...
 * - Data nodes: Wider (150-200px) to fit long field names
 * - Audio nodes: Narrower (110-140px) with progress bars at bottom
...
export class PatchViz {
    constructor(svgElementId) {
        ...
        // Hover state management (prevent flicker when moving between nodes)
    ...
    render(numericPaths, mappings, parameterMapper, isPlaying = false) {
        ...
        const width = parseInt(this.svg.style('width')) || 800;
    ...
    _createNodes(numericPaths, parameterMapper, svgWidth) {
        ...
        const columnGap = (svgWidth - (2 * margin)) * 0.15; // Reduced to increase distance between columns
    ...
    _applyNodeInteractivity(nodeGroups, mappings, isPlaying) {
        // Capture reference to nodes array and viz instance (inside .each(), 'this' refers to DOM element)
        const nodes = this.nodes;
        const viz = this; // Capture PatchViz instance for timeout access
        ...
        nodeGroups.each(function(d) {
            ...
            nodeGroup
                .on('mouseenter', function() {
                    // Cancel any pending mouseleave timeout (prevents flicker)
                    if (viz._hoverTimeout) {
                        clearTimeout(viz._hoverTimeout);
                        viz._hoverTimeout = null;
                    }
                ...
                .on('mouseleave', function() {
                    // Delay clearing hover effects to prevent flicker when moving between nodes
                    // If user hovers over another node within 50ms, the timeout is cancelled
                    viz._hoverTimeout = setTimeout(() => {
                        // Clear hover effects
                        ...
                        viz._hoverTimeout = null;
                    }, 50); // 50ms delay prevents flicker during quick mouse movements
                });
        });
    }
            ...
            if (node && value !== undefined && value !== null) {
                ...
                if (!barEl.empty() && typeof value === 'number') {
                    ...
                    if (mapping) {
                        ...
                        const nodeWidth = svgWidth < 600 ? 110 : 140;
