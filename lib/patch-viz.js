 ...
 * - Layout: Columns centered with fixed distance between them (250-500px depending on viewport)
 * - Responsive: Mobile (250px), Tablet (400px), Desktop (500px) column spacing
 * - Safeguards: Minimum distance enforced to prevent overlap
 */
...
export class PatchViz {
    ...
    render(numericPaths, mappings, parameterMapper, isPlaying = false) {
        ...
        // Explicitly set SVG width to match container for predictability
    ...
    _createNodes(numericPaths, parameterMapper, svgWidth) {
        ...
        // LEFT-SHIFTED COLUMN POSITIONING
        // Shift center point to the left for better visual balance
        const centerOffset = -30; // Negative shifts left
        const center = (svgWidth / 2) + centerOffset;
        const nodeWidth = svgWidth < 600 ? 165 : 220; // Node width (same for both columns)
        
        // Distance between column centers (not edges)
        // Responsive: smaller viewports need tighter spacing
        let distanceBetweenColumns;
        if (svgWidth < 600) {
            distanceBetweenColumns = 250; // Mobile: tight spacing
        } else if (svgWidth < 1000) {
            distanceBetweenColumns = 400; // Tablet: medium spacing
        } else {
            distanceBetweenColumns = 500; // Desktop: comfortable spacing
        }
        
        // Ensure columns don't overlap (minimum distance = nodeWidth + 50px gap)
        const minDistance = nodeWidth + 50;
        distanceBetweenColumns = Math.max(distanceBetweenColumns, minDistance);
        
        // Position columns symmetrically around center
        const leftX = center - (distanceBetweenColumns / 2);
        const rightX = center + (distanceBetweenColumns / 2);
        
        console.log(`📐 Layout: width=${svgWidth}, center=${center}, distance=${distanceBetweenColumns}, leftX=${leftX.toFixed(0)}, rightX=${rightX.toFixed(0)}, nodeWidth=${nodeWidth}`);
    ...
    _drawNodes(container, svgWidth, mappings, isPlaying) {
        ...
        const nodeWidth = svgWidth < 600 ? 165 : 220;
